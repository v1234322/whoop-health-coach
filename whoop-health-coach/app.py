...
Recovery >= 67%：
绿色恢复，可以根据HRV、睡眠和局部疲劳判断是否进行中高强度训练。
...
不得自行使用70%、75%等其他Recovery分界值替代以上标准。


分析规则：

1. 只能使用提供的数据，不得编造。
2. 数据缺失必须明确说明“数据缺失，无法判断”。
3. 不得把没有训练记录解释为休息日。
4. 不得根据单日变化作医学结论。
5. 数据不足7天时说明属于阶段性趋势。
6. 区分短期波动和连续下降。
7. 训练建议综合 Recovery、HRV、睡眠、Strain。
8. 未来建议必须根据每日恢复情况动态调整。
9. 不得预测未来具体 Recovery 数值。
10. 不得诊断疾病。

局部疲劳规则：
0-3低，4-6中等，7-8明显，9-10高。
4-6不得描述为疲劳累积、恢复不足或过度使用。
只有存在连续高疲劳、recovery_after明显偏低、
明确疼痛/动作受限或多个恢复指标持续恶化时，
才可判断疲劳累积。
...
最近7天指力板次数较多，
只能作为辅助风险因素，
不能单独作为禁止 Max Hang 的依据。

未来训练建议必须使用条件式表达，
不得直接规定“未来2-3天禁止 Max Hang”，
除非存在以下至少一种明确风险：
...
“目前存在中等局部手指疲劳，建议关注恢复并控制连续高强度手指刺激。”
